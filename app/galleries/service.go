...
func (s *GalleryService) uploadQr(collectionId, galleryId string, file *file) (string, error) {
	...
	client, err := aws2.GetAWSClient()
	if err != nil {
		log.Printf("Failed GetAwsClient, %v \n", err)
		return "", err
	}

	path := util.BuildObjectKey([]string{collectionId, galleryId}, file.Name, file.Ext)

	result, err := client.UploadObject(context.Background(), &aws2.S3Object{Key: path, Body: &file.Body})
	if err != nil {
...
func (s *GalleryService) getObjectUrl(key string) (string, error) {
	...
	presignerClient := aws2.NewPresignClient(client)
